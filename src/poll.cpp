...
{
	poll[i] = poll[*fd_count - 1]; /*copiamos el del final sobre este*/
...
{	
	...
	while (1)
	{
		...
		for (int i = 0; i < data->fd_size; i++)
		{
			if (data->poll[i].revents & POLLIN) // hay alguien listo para leer = hay un intento de conexion
			{
				if (check_if_listener(data->poll[i].fd, list))
				{
					/*si es verdad, el fd corresponde a un socket servidor y hay que aceptar nueva conexion*/
